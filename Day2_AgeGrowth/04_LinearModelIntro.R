...
library(FSA)         # for filterD(), fitPlot
library(dplyr)       # for %>%
...
plot(logW~logL,data=ruf90,pch=19,col=rgb(0,0,0,1/4),ylab="log Weight (g)",xlab="log Total Length (mm)")
lines(ys~xs,lwd=2)

plot(wt~tl,data=ruf90,pch=19,col=rgb(0,0,0,1/4),ylab="Weight (g)",xlab="Total Length (mm)")
btxs <- 10^xs
btys <- 10^ys
...
# base plot
clrs <- c(rgb(0,0,0,1/5),rgb(0,0,1,1/5))
